...
BOOL CHoverButton::LoadBitmap(CString FileName)
{
	mybitmap.Attach(::LoadImage(::AfxGetInstanceHandle(),FileName, IMAGE_BITMAP,0,0,LR_LOADFROMFILE));
	BITMAP	bitmapbits;
	mybitmap.GetBitmap(&bitmapbits);
	m_ButtonSize.cy=bitmapbits.bmHeight;
	m_ButtonSize.cx=bitmapbits.bmWidth/4;
